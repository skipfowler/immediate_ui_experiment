...
void DemoWindow::Update() {
  ...
  if (submitting && gauge && progress_horizontal) {
    ...
    const float progress = Math::Min(
        float(GetSystemInterface()->GetElapsedTime() - submitting_start_time) /
            progressbars_time,
        2.f);
    ...
    if (progress > 1.0f) label += "... Placed! Assembling message";
    if (progress < 2.0f)
...
void DemoWindow::ProcessEvent(Rml::Event &event) {
  ...
  switch (event.GetId()) {
    case EventId::Keydown: {
      ...
      if (key_identifier == Rml::Input::KI_ESCAPE) _backend->RequestExit();
    } break;
...
Rml::ElementDocument *DemoWindow::GetDocument() { return document; }
...
void DemoWindow::SetBackend(Rml::SharedPtr<RmlUi::Backend::glfwGL3> backend) {
  _backend = backend;
}